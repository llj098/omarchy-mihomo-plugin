...
The complete URL is saved in `source.url` with mode 0600 but is never returned by the status helper. Importing the exact URL again atomically updates its existing entry; byte-identical content returns `unchanged` without rewriting the file. URL subscriptions also show an Omarchy-standard refresh button in the subscription header: a `PanelActionButton` (the official row-edge action component) using the standard refresh glyph, whose tooltip reads `Update subscription` and becomes `Updating` while the request runs. A successful update shows a brief notice (`Subscription updated`/`unchanged`) that clears automatically after three seconds; the durable result is the refreshed list itself (updated node count and modification time). Failures show an urgent message that also clears automatically. This transient-notice pattern follows Omarchy's own `failureTimer` behaviour rather than a persistent banner or a toast component, which Omarchy does not provide. Local subscriptions have no update button because re-importing a local file deliberately creates a new entry instead of updating one. Updating only rewrites the stored subscription data; if Mihomo is running, the active runtime keeps using the configuration generated when the node was started, so re-selecting the node is required to pick up the new subscription. Local files deliberately have no source metadata or deduplication and create a new entry each time. Entries and the data directory use modes 0600 and 0700 respectively.
...
Clicking an inline node generates an isolated runtime configuration and starts a transient user service named `fatlj-mihomo.service`. Runtime settings default to mixed port `7891` and localhost-only binding, leaving any existing service on `7890` untouched. Any port in `1024..65535`, including `7890`, and **Allow LAN** are drafts until **Apply** is clicked. An occupied target port is rejected without interrupting the running Mihomo instance. Settings are atomically saved mode 0600 at `~/.local/state/fatlj.mihomo/settings.json`; Apply restarts the same selected node only when this plugin runtime is active, otherwise it affects the next start. A failed restart restores the previous settings and attempts to restore the previous running endpoint. The panel highlights the active node, shows the runtime endpoint, and provides **Stop Mihomo**. If the selected node name is visually truncated, hovering it uses Omarchy's standard themed `PanelToolTip` to reveal the complete name.

The main runtime always exposes its Controller through `~/.local/state/fatlj.mihomo/runtime/controller.sock`, inside the mode-0700 state directory; no TCP Controller is opened. While the panel is open, it reads active connections, current transfer rates, totals, and selected-node health from that running process every two seconds. Mihomo's valid `connections: null` response is treated as zero active connections without discarding transfer totals. Closing the panel stops all polling. Loading or rebuilding the plugin performs one basic systemd/state-file status read so the bar icon survives Shell restarts; it does not access the Controller or start periodic background work. Opening the panel performs one Controller latency test for the active node so the Latency field has a current sample; it is not repeated while the panel remains closed. If that node does not respond or exceeds 1000 ms, one asynchronous recommendation run starts temporary, listener-free Mihomo instances outside the watched plugin tree. Each imported subscription is tested independently, the three fastest responsive inline nodes are shown in a height-bounded **Recommend** section, and those rows reuse the normal proxy-node component and click-to-start behavior. A responsive active node at or below 1000 ms never starts this work; if the active node becomes healthy after a switch, any in-flight recommendation test is terminated and the section is cleared. The temporary processes never replace or stop the main runtime and are terminated after their tests; Linux parent-death signaling also terminates them if the Shell kills the helper during a reload. Manual group speed tests use the main Controller when the tested subscription is active, and they test every member of the subscription's original group individually (`/proxies/{name}/delay`). The native `/group/{name}/delay` endpoint is deliberately not used for these tests because the running runtime narrows the selected node's group to that one node, which would make every other original member look like a timeout. Otherwise they automatically launch one listener-free temporary Mihomo, so a group can be tested before selecting a node or while another subscription is active. Temporary tests wait until the requested proxies appear in the Controller inventory; an all-node failure that returns in under one second is treated as a cold-start race and retried once.
...
printf '%s\n' '{"subscriptionId":"url-<sha256-of-exact-url>"}' | ./subscription/control.py update
```
