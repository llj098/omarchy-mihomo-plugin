...
grep -Fq 'readonly property string pluginVersion: "0.10.6"' "$PANEL" || fail "panel plugin version does not match the manifest"
...
grep -Fq 'command: [root.subscriptionControlScript, "update"]' "$PANEL" || fail "subscription update is not wired"
...
grep -Fq 'interval: 3000' "$PANEL" || fail "subscription notice timer interval is missing"
grep -Fq 'subscriptionNoticeTimer.restart()' "$PANEL" || fail "update/import notices do not auto-clear"
grep -Fq 'root.subscriptionMessage = result.action === "updated" ? "Subscription updated"' "$PANEL" || fail "successful update has no transient notice"
