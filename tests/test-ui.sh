...
grep -Fq 'text: "󰓅"' "$PANEL" || fail "latency action does not use the official network meter icon"
grep -Fq 'text: groupTestButton.testing ? "Testing" : "Speed Test"' "$PANEL" || fail "latency action tooltip is missing"
